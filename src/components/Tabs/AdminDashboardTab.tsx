...
export const AdminDashboardTab: React.FC<AdminDashboardTabProps> = ({ 
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    ...
    if (clean === '10111') {
      ...
      showToast('🔓 Admin Workspace unlocked with authorized PIN: 10111');
    } else {
      setPinError(true);
      setPinInput('');
    }
  ...
  // Reset / Quick fill authorized PIN to 10111
  const handleResetPinToDefault = async () => {
    try {
      ...
      setPinInput('10111');
      setPinError(false);
      showToast('🔑 Authorized PIN set to: 10111');
    } catch {
      localStorage.setItem('studyhub_admin_pin', '10111');
      setPinInput('10111');
      setPinError(false);
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-md mx-auto my-12 p-6 sm:p-8 bg-[#FBF9F5] dark:bg-[#111612] border border-[#D9D1C7] dark:border-[#2D382F] rounded-3xl shadow-lg text-center space-y-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-[#2D362E] dark:text-white">Admin Workspace Protected</h2>
          <p className="text-xs text-[#736B5E] dark:text-[#A6C4A7] mt-1">
            Enter the authorized 5-digit Admin PIN (<strong>10111</strong>). No other PINs allowed.
          </p>
        </div>
        ...
        <form onSubmit={handlePinSubmit} className="space-y-4">
          <div className="relative">
            <input
              ...
              placeholder="Enter 5-digit PIN (10111)"
            ...
            {pinError && (
              <p className="text-[11px] text-red-500 font-semibold mt-1">
                Incorrect PIN. Only the authorized Admin PIN (10111) is allowed.
              </p>
            )}
          </div>
          ...
          >
            ...
            <span>Unlock with PIN 10111</span>
          </button>
        </form>
      </div>
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-6 bg-gradient-to-r from-[#2D362E] to-[#455447] text-white rounded-3xl shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            title="Authorized Admin PIN: 10111"
          >
